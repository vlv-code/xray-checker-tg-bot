...
Публичные эндпоинты не требуют аутентификации или открываются при включении публичного статуса.
...
Простой эндпоинт проверки работоспособности. Всегда доступен без аутентификации (для балансировщиков и проверок оркестратора).
...
:::note[Особенности аутентификации]
Этот эндпоинт доступен без аутентификации, если `METRICS_PROTECTED=false` ИЛИ `WEB_PUBLIC=true`. Если `METRICS_PROTECTED=true` и `WEB_PUBLIC=false`, для доступа к эндпоинту требуется HTTP Basic Auth.
:::
...
**Аутентификация:** `Authorization: Bearer <REPORT_TOKEN>`. Этот эндпоинт предназначен исключительно для push-отчетов нод, полностью минует Basic Auth и проверяет только Bearer-токен по списку зарегистрированных нод на мастере.
...
- `/health` всегда доступен без аутентификации (для проверок балансировщиков и оркестраторов).
- `/api/v1/public/proxies` доступен без аутентификации при `WEB_PUBLIC=true` (или при `METRICS_PROTECTED=false`).
- `/api/v1/nodes/report` аутентифицируется исключительно через заголовок `Authorization: Bearer <токен>` и не требует Basic Auth.
